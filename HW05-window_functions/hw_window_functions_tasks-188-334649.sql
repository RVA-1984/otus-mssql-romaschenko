...
count (QuantityPerOuter) over () as 'Общее количество товаров' from Warehouse.StockItems

-- посчитайте общее количество товаров в зависимости от первой буквы названия товара
select StockItemID, StockItemName, Brand, UnitPrice, 
ROW_NUMBER () OVER (PARTITION BY left(StockItemName,1) order by StockItemName), 
count (QuantityPerOuter) over () as 'Общее количество товаров', 
count (QuantityPerOuter) over (PARTITION BY left(StockItemName,1) 
...
ntile(30) OVER (order by TypicalWeightPerUnit) as 'Группа товаров по полю вес'
from Warehouse.StockItems
...
; with CTE as (
				...
				,o.UnitPrice
				,o.StockItemID
				,max(s.OrderDate) OVER (order by o.UnitPrice desc) as 'Дата покупки товара'
				,DENSE_RANK() OVER (order by o.UnitPrice desc) as SORT
